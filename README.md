...
A last-ditch failsafe using the [`item_used_on_block`](https://minecraft.wiki/w/Advancement_definition#minecraft:item_used_on_block) advancement trigger to restore the item would suffice to save the player from such inexplicable strife, but still breaks Survival immersion unless you devise some *very* dense logic involving `loot replace` (and we'd hate for the player to think this interaction was intended).
